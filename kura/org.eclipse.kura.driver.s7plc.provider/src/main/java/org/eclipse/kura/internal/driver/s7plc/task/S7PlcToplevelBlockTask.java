    ...
    public void processBuffer() throws IOException {
        if (getMode() == Mode.READ) {
            logger.debug("Reading from PLC, DB{} offset: {} length: {}", this.areaNo, getStart(),
                    getBuffer().getLength());
            this.driver.read(this.dbType, this.areaNo, getStart(), ((ByteArrayBuffer) getBuffer()).getBackingArray());
        } else {
            logger.debug("Writing to PLC, DB{} offset: {} length: {}", this.areaNo, getStart(),
                    getBuffer().getLength());
            this.driver.write(this.dbType, this.areaNo, getStart(), ((ByteArrayBuffer) getBuffer()).getBackingArray());
        }
    }
