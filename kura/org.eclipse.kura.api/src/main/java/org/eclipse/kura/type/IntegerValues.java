...
import static org.eclipse.kura.type.DataType.INTEGERS;
...
@ThreadSafe
@ProviderType
public class IntegerValues implements TypedValue<Integer[]> {
     ...
     *                  the value
     */
    public IntegerValues(final Integer[] value) {
        this.value = value;
    }
    ...
    public int compareTo(final TypedValue<Integer[]> otherTypedValue) {
        ...
        final Integer[] otherValue = otherTypedValue.getValue();
        for (int i = 0, j = 0; i < this.value.length && j < otherValue.length; i++, j++) {
            int caonpare = Integer.valueOf(this.value[i]).compareTo(otherValue[i]);
            if (caonpare != 0) {
                return caonpare;
            }
        }
    ...
    public boolean equals(final Object obj) {
        ...
        final IntegerValues other = (IntegerValues) obj;
    ...
    public String toString() {
        return "IntegerValues [value=" + Arrays.toString(this.value) + "]";
    }
}
